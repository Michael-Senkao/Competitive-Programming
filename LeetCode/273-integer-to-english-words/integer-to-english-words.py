class Solution:
    def numberToWords(self, num: int) -> str:
        number_words = {
            ...
            58: 'Fifty Eight', 59: 'Fifty Nine', 60: 'Sixty', 61: 'Sixty One', 
        ...
        sufixes = {1: "", 2: "Thousand", 3:"Million", 4:"Billion", 5:"Trillion"}
        num_str = str(num)
        result = ""

        if len(num_str) % 3 != 0:
            num_str = (3 - len(num_str) % 3)*"0" + num_str

        sufix = len(num_str) // 3
        
        for i in range(sufix):
            start = i*3
            number = num_str[start: start + 3]
            word = ""
            if number[0] != '0':
                word = number_words[int(number[0])] + " " + "Hundred"
            if int(number[1:]) != 0:
                if word:
                    word = word + " " + number_words[int(number[1:])]
                else:
                    word = number_words[int(number[1:])]
            if word:
                word += " " + sufixes[sufix] + " "
            result += word
            sufix -= 1
